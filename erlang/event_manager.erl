...
loop(State) ->
  receive
    {request, From,{swap_handler,OldHandler,NewHandler}} ->
      InitData = OldHandler:terminate(),
      {Reply, DeleteState} = handle_msg({delete_handler,OldHandler}, State),
      {Reply, AddState} = handle_msg({add_handler,NewHandler,InitData}, DeleteState),
      reply(From, Reply),
      loop(AddState);
    {request, From, Msg} ->
